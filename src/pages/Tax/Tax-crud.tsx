...
import { GetTaxById, type TaxType } from '@/actions/accounting/tax-action';
import TaxForm from './Tax-form';

export default function TaxCRUD() {
    const axios = useAxiosInstance();
    const [tax, setTax] = React.useState<TaxType | undefined>();
    ...
    React.useEffect(() => {
        async function getData() {
            try {
                ...
                await GetTaxById(axios, Number(id))
                    .then((res) => {
                        if (res.IsError) {
    ...
    if (pageAction === PageAction.view) {
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10 ">
                <h1 className="font-bold text-xl text-left w-full mb-2">Tax</h1>
                <TaxForm data={tax} pageAction={PageAction.view} />
            </div>
        );
    } else if (pageAction === PageAction.add) {
        return (
            <>
                <BreadcrumbAddNew pageName="New Tax" isShowAddNewButton={false} handleNavigateToAddNewPage={() => { }} />
                <div className="flex flex-col gap-10">
                    <TaxForm data={tax} pageAction={PageAction.add} />
                </div>
            </>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <TaxForm data={tax} pageAction={PageAction.edit} />
            </div>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <TaxForm data={tax} pageAction={PageAction.delete} />
            </div>
        );
    } else {
        return (
